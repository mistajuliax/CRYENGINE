...
class Context(ctx):
	...
	def recurse_subdirs(self, name=None, mandatory=True, once=True):
		thisdir = self.path.abspath()
		subdirs = [x for x in os.listdir(thisdir) if os.path.isdir(os.path.join(thisdir, x))]
		self.recurse(subdirs, name, mandatory, once)
