...
def memoize(fun):
	...
	def wrap(*args, **kwargs):
		if args not in wrap.__cache__:
			wrap.__cache__[args] = fun(*args, **kwargs)
		return wrap.__cache__[args]

	wrap.__cache__ = cache
	return wrap		
	
def memoize_ignore_first_arg(fun):
	"""
	Decorator: let a function cache its results. First parameter is not taking into account for caching and should be read only
	...
	def wrap(first_arg, *args, **kwargs):
		if args not in wrap.__cache__:
			wrap.__cache__[args] = fun(first_arg, *args, **kwargs)
		return wrap.__cache__[args]
...
def run_once(fun):
	"""
	Decorator: ensure function is only executed once during the complete process
	...
	@wraps(fun)
	def wrap(*args, **kwargs):
		if not wrap.__has_run__:
			wrap.__has_run__ = True
			return fun(*args, **kwargs)

	wrap.__has_run__ = False
	return wrap
