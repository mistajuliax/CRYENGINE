...
def module_extensions_openvr(ctx, kw, entry_prefix, platform, configuration):
	...
	kw[f'{entry_prefix}defines'] += [ 'USE_OPENVR', 'INCLUDE_OPENVR_SDK' ]
	kw[f'{entry_prefix}includes'] += [
	    ctx.CreateRootRelativePath('Code/SDKs/OpenVR/headers')
	]
	kw[f'{entry_prefix}lib'] += [ 'openvr_api' ]

	if platform == 'win_x86':
		kw[f'{entry_prefix}libpath'] += [
		    ctx.CreateRootRelativePath('Code/SDKs/OpenVR/lib/win32')
		]
	elif platform == 'win_x64':
		kw[f'{entry_prefix}libpath'] += [
		    ctx.CreateRootRelativePath('Code/SDKs/OpenVR/lib/win64')
		]

	kw[f'{entry_prefix}features'] += [ 'copy_openvr_binaries' ]
...
def feature_copy_openvr_binaries(self):
	...
	if platform not in bin_location:
		Logs.error(f'[ERROR] OpenVR is not supported on plaform by WAF: {platform}')
