...
def module_extensions_oculus_audio_wwise(ctx, kw, entry_prefix, platform, configuration):	
	...
	if platform != 'project_generator':
		kw[f'{entry_prefix}features'] += [ 'copy_oculus_audio_wwise_binaries' ]
...
def feature_copy_oculus_audio_wwise_binaries(self):
	...
	if platform not in ['win_x86', 'win_x64']:
		Logs.error(
		    f'[ERROR] Oculus Wwise Audio is not supported for plaform {platform} by WAF'
		)

	target_platform = 'x64' if platform == 'win_x64' else 'Win32'
	bin_name = 'OculusSpatializer.dll'
	src_path = bld.CreateRootRelativePath(
	    f'Code/SDKs/audio/oculus/wwise/{target_platform}/bin/plugins/{bin_name}')
	...
	if os.path.isfile(src_path):
		...
		Logs.warn(f'[WARNING] Oculus Wwise Audio DLL not found at: {src_path}')
