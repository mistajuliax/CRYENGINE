...
def module_extensions_steam(ctx, kw, entry_prefix, platform, configuration):
	...
	kw[f'{entry_prefix}defines'] += [ 'USE_STEAM=1' ]
	kw[f'{entry_prefix}includes'] += [
	    ctx.CreateRootRelativePath('Code/SDKs/Steamworks/public')
	]

	if platform == 'win_x86':
		kw[f'{entry_prefix}libpath'] += [
		    ctx.CreateRootRelativePath('Code/SDKs/Steamworks/redistributable_bin')
		]
	elif platform == 'win_x64':
		kw[f'{entry_prefix}libpath'] += [
		    ctx.CreateRootRelativePath(
		        'Code/SDKs/Steamworks/redistributable_bin/win64')
		]

	if platform != 'project_generator':
		kw[f'{entry_prefix}features'] += [ 'copy_steam_api_binaries' ]
...
def feature_copy_steam_api_binaries(self):
	...
	if platform not in bin_location:
		Logs.error(f'[ERROR] Steam is not supported on plaform by WAF: {platform}')
