...
class Local_CommandCoordinator(object):
	...
	def execute_command(self, cmd, **kw):
		try:
			...
			raise Errors.WafError(f'Execution failure: {str(e)} ({cmd})', ex=e)
...
def get_ib_licence_string():
	return subprocess.check_output(
	    [f'{str(get_ib_folder())}/xgconsole.exe', '/QUERYLICENSE'])
...
def execute_waf_via_ib(bld):	
	...
	if 'PlayStation' not in result and 'orbis' in bld.cmd:
	...
	cmd_line_args = [arg for arg in sys.argv[1:] if arg != 'generate_uber_files']
	command_line_options = (' '.join(cmd_line_args) +
	                        ' --internal-dont-check-recursive-execution=True')
	# Build Command Line
	cry_waf = bld.path.make_node('Code/Tools/waf-1.7.13/bin/cry_waf.exe')
	command = f'{cry_waf.abspath()} {command_line_options}'
	...
	if (has_dev_tools_acceleration_license and not has_make_and_build_license)  or 'linux' in bld.variant or 'android' in bld.variant or 'cppcheck' in bld.variant:
		...
		try:
			p = subprocess.Popen([
			    f'{str(ib_folder)}/BuildConsole.exe',
			    f"/command={command}",
			    "/useidemonitor",
			    "/nologo",
			    allow_local,
