...
def _get_project_overrides(ctx, target):	
	...
	if ctx.cmd in ['generate_uber_files', 'msvs', 'configure']:
		return ({}, {})
	...
	vs_valid_spec_for_build = f'[{vs_spec}] {vs_configuration}|{vs_platform}'

	vcxproj_file = (
	    ctx.get_project_output_folder().make_node(f'{target}.vcxproj').abspath())
	...
	try:
		...
		Logs.warn(
		    f'warning: Unable to parse .vcxproj file to extract configuration overrides. [File:{vcxproj_file}] [Exception:{sys.exc_info()[0]}]'
		)
		return ({}, {})
...
def get_solution_overrides(self):	
	...
	if self.cmd in ['generate_uber_files', 'msvs']:
		return {}
	...
	try:
		...
		Logs.debug(
		    f'warning: Unable to parse .sln file to extract configuration overrides: [File:{sln_file}] [Exception:{sys.exc_info()[0]}]'
		)
		return {}
	...
	vs_build_configuration = f'[{vs_spec}] {vs_configuration}|{vs_platform}'
